// Unified Robust Storage & Database Engine

const KEYS = {
  PROFILE: 'cyber_portfolio_profile_v2',
  PROJECTS: 'cyber_portfolio_projects_v2',
  INITIALIZED: 'cyber_portfolio_init_v2',
  TOKEN: 'cyber_admin_token',
  PASS: 'cyber_active_pass'
};
...
class StorageEngine {
  ...
  saveProjects(projectsArray) {
    localStorage.setItem(KEYS.PROJECTS, JSON.stringify(projectsArray));
    localStorage.setItem(KEYS.INITIALIZED, 'true');
  }
  ...
  resetToDefaults() {
    ...
    localStorage.setItem(KEYS.INITIALIZED, 'true');
    return { profile: INITIAL_PROFILE, projects: INITIAL_PROJECTS };
  }
}
